...
import { FindUserApi, FindUserErrorApi } from "../../../api/user.api";
...
export function EditUserModal({ userId, onClose }: { userId: number; onClose: () => void }) {
  const { data: user } = useQuery({
    queryKey: ["editSaveUser", userId],
    queryFn: async () => FindUserApi({ userId: Number(userId) }),
  });

  const addMutation = useMutation({
    mutationFn: async (add: UserFormValues) => await FindUserErrorApi({ userId: add.id }),
  });
